...
export async function POST(req: NextRequest, ctx: { params: Promise<{ ext: string }> }) {
  ...
  if (!authHeader || !public_key || !private_key) {
    return NextResponse.json({ error: "Missing TELXIO credentials" }, { status: 500, headers });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "Both GET and POST methods failed", upstream: data }, { status: res?.status || 502, headers });
  } catch (err: any) {
