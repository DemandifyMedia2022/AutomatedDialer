            ...
            <article key={exchange.id} className="rounded-sm bg-background p-4">
              ...
              {exchange.assistantMessage && !isGenerating ? (
                <div className="mt-4 flex items-center justify-start gap-2">
                  ...
                  <button
                    type="button"
                    className={`border-none bg-transparent rounded-md px-2.5 py-1.5 text-muted-foreground cursor-pointer flex items-center justify-center gap-2 transition-all duration-150 hover:bg-blue-50 dark:hover:bg-slate-900 disabled:text-foreground disabled:cursor-not-allowed ${copiedExchangeId === exchange.id
...
const Sidepanel = memo(function Sidepanel({
  ...
  return createPortal(
    <div
      className={`fixed inset-0 z-50 ${variant === "inline"
        ? "bg-transparent dark:bg-transparent md:p-0"
        : "bg-black/20 dark:bg-black/60 md:p-4"
        } flex items-center justify-end pointer-events-none ${isVisible ? "animate-in fade-in-0" : "animate-out fade-out-0"
        }`}
      style={{ animationDuration: "0.2s" }}
    >
      <div
        className={`bg-background h-screen w-full md:h-full flex flex-col pointer-events-auto transition-all duration-300 ease-out ${variant === "inline" ? "rounded-none border-l border-border" : "md:rounded-lg shadow-2xl"} ${isVisible
